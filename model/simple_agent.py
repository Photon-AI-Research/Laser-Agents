...
class SimpleAgent():
    def __init__(self, target_size, input_size = None, hidden_size=None, num_hidden=1, activation=torch.tanh, lr=1e-4, loss_f=torch.nn.MSELoss(), conv=False, kernel_size=7, last_layer_activation = True, norm=False, clip_norm=False, minmax_norm=True, buffer_size= 50000, pwl=False, xn=None, xp=None):
        ...
        if conv: 
            ...
            self.agent = MLP(input_size, target_size, hidden_size, num_hidden, activation = activation, last_layer_activation=last_layer_activation, norm=norm, clip_norm=clip_norm, minmax_norm=minmax_norm, pwl=pwl, xn=xn, xp=xp).to(device)
        # Experience buffer
    ...
    def pretrain(self, train_loader, num_epochs=1000, log_writer=None, num_total_plots = 1, num_plots = 1, plot = False, downsampling_rate=None, log_every=1000, model_name = None, val_loader=None):
        # Loss track
        loss_evol = []
        # Pretrain
            ...
            if log_writer != None and epoch % log_every == 0:
                if log_writer[0] == 'wb':
                    ...
                    if val_loader != None:
                        loss_val = []
                        # Iterations
                        for val_state, val_action in val_loader:
                            self.optim.zero_grad()
                            val_predicted_action = self.agent(val_state)
                            with torch.no_grad():
                                val_loss = self.loss_f(val_predicted_action, val_action)
                            loss_val.append(val_loss.item())
                        val_loss = np.mean(loss_val)
                        log_writer[1].log({'Validation loss': val_loss, 'Epoch': epoch})
    ...
    def surrogate_learn_from_exp(self, surrogate, train_params, prior, ssim=False, rollout=1, surrogate_rollout=1, surrogate_step=True):
        ...
        if surrogate_step:
            for i in range(surrogate_rollout):
