...
for i in range(10000):
    ...
    out = agent.agent(states[idx,:].unsqueeze(0).float().to(device))
    loss = agent.loss_f(out,pretrain_goal)
...
np.save("env_surrogate_test_{}_{}_{}_{}_{}".format(num_episodes, train_params["batch_size"], agent_lr, surrogate_lr, last_layer_act_agent),{"loss": loss, "agent":agent.agent.state_dict(),"surrogate":surrogate.model.state_dict(), "train_params":train_params, "lr":(agent_lr, surrogate_lr)})