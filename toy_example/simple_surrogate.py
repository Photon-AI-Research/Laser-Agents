...
class SimpleSurrogate():
    def __init__(self, target_size, hidden_size=None, num_hidden=1, activation=torch.tanh, lr=1e-4, loss_f=torch.nn.MSELoss()):
        hidden_size = target_size if hidden_size == None else hidden_size
        # Model
        self.model = MLP(target_size*2, target_size, hidden_size, num_hidden, activation = activation).to(device)
        # Experience buffer
