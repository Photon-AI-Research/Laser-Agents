...
def create_window(window_size, channel):
    ...
    _2D_window = _1D_window.float().unsqueeze(0).unsqueeze(0)
    window = Variable(_2D_window.expand(channel, 1, 1, window_size).contiguous())
    return window

def _ssim(img1, img2, window, window_size, channel, size_average = True):
    window = window[0].to(device)
    #print(window.shape)
    mu1 = F.conv1d(img1, window, padding = window_size//2, groups = channel)
